...
class AuthController extends Controller
{
    ...
    public function reset_password_page(): View
    {
    ...
    public function reset_password_link(Request $request): RedirectResponse
    {
        $request->validate([
            'email' => 'required|email'
        ]);
    ...
    {
        $data = User::query()->where('token', $token)->where('email', $email)->first();
        if (!$data) {
